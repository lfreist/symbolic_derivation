...
TEST(TokenizerTest, next_token) {
  ...
  }
  {
    ...
    auto token = tokenizer.next_token();
    EXPECT_TRUE(token.has_value());
    EXPECT_EQ(token.value(), fsd::Token(std::string_view(input.begin(), input.begin() + 1), fsd::TokenType_TP::LEFT_PAREN));
  }
  {
    const std::string input(")");
    fsd::Tokenizer tokenizer(input);
    auto token = tokenizer.next_token();
    EXPECT_TRUE(token.has_value());
    EXPECT_EQ(token.value(), fsd::Token(std::string_view(input.begin(), input.begin() + 1), fsd::TokenType_TP::RIGHT_PAREN));
  }
  {
    ...
    auto token = tokenizer.next_token();
    EXPECT_TRUE(token.has_value());
    EXPECT_EQ(token.value(), fsd::Token(std::string_view(input.begin(), input.begin() + 4), fsd::TokenType_TP::LITERAL));
  }
  {
